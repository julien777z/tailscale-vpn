...
- Agent B: List paths to `.claude/CLAUDE.md` (if present) and any `CLAUDE.md` under PR-touched directories; paths only, not contents.
...
4. For each issue found in step 3, launch a **parallel Haiku agent** that scores it 0–100. Give this rubric to the agent verbatim:
   ...
   - 25: Uncertain — might be real, but the agent could not verify it. If stylistic, it is not explicitly called out in CLAUDE.md.
   - 50: Likely real but minor — the agent verified it, but it is a nitpick or happens rarely.
   - 75: High confidence — verified, likely hit in practice, important or explicitly mentioned in CLAUDE.md.
   - 100: Certain — confirmed real, happens frequently, directly evidenced.

   For CLAUDE.md issues: double-check that the CLAUDE.md actually calls out that specific issue before scoring above 50.
...
- Cite and link every issue (including the relevant CLAUDE.md when applicable).
...
1. ✅ **Missing auth check on mutating route** *(fixed)* — Added `CurrentUser` dependency consistent with sibling routes. (CLAUDE.md says "…")
